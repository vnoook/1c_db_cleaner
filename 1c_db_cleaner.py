# TODO
# 0) исправить выдачу размера файла в письме, сделать число дробным без округления, строка 326
# 1) проанализировать размеры файлов, если отличается один от остальных, то разобраться почему
...
# 7) добавить в письмо инфу про файлы, которые неполучилось удалить
# 8) сделать возможность отправлять письма на несколько ящиков
...
# 3) проверяет на целостность архива путём поиска файла 1Cv8.1CD внутри архива
# 4) следит чтобы в папке не было больше quantity_files_in_dir файлов архивных баз 1с и удаляет старые по дате
# 5) пишет письмо после каждого исполнения — статистика действий и свободное место на диске
# ...
